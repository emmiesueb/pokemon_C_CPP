...
void new_hiker()
{
  ...
  c->party[0] = generate_pkmn(min_lvl, max_lvl);
  c->party[1] = generate_pkmn(min_lvl, max_lvl);
  c->party[2] = generate_pkmn(min_lvl, max_lvl);
  c->num_in_party = 3;
  c->num_pkmn_defeated = 0;
  c->base_money = 100;
...
void init_pc()
{
  int x, y;
  world.pc = new Pc;
  world.pc->party[0] = generate_pkmn(1, max_lvl);
  world.pc->party[1] = generate_pkmn(1, max_lvl);
  world.pc->party[2] = generate_pkmn(1, max_lvl);
  world.pc->num_in_party = 3;
  ...
  world.pc->num_in_pcc = 0;
  world.pc->bag[0] = new Items;
  world.pc->bag[0]->num = 5;
  world.pc->bag[0]->identifier = "Potion";
  world.pc->bag[1] = new Items;
  world.pc->bag[1]->num = 5;
  world.pc->bag[1]->identifier = "Revive";
  world.pc->bag[2] = new Items;
  world.pc->bag[2]->num = 10;
  world.pc->bag[2]->identifier = "Pokeball";
  world.pc->wallet = 3000;
...
int new_map(int teleport)
{
  ...
  world.world[world.cur_idx[dim_y]][world.cur_idx[dim_x]] = world.cur_map = new Map;
  ...
  // determine min level based on distance:
  if (d > 200) {
    min_lvl = (d - 200) / 2;
    max_lvl = 100;
  }
...
void usage(char *s)
{
  std::cerr << "Usage: " << s << "[-s]--seed <seed>]" << endl;
